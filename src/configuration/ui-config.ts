...
export module UiConfig {
    export const CONTAINER_PROVIDER_KEY = 'session_provider';
    export const TEST_PLATFORM_KEY = 'test_platform';
    
    export async function provider(provider?: string): Promise<string> {
        if (provider) {
            TestConfig.setGlobalValue(CONTAINER_PROVIDER_KEY, provider);
        }
        return await TestConfig.getValueOrDefault(CONTAINER_PROVIDER_KEY);
    }
